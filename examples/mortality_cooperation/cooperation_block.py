...
Each simulated day, the agent faces a cooperation dilemma:
  - COOPERATE: pay 15 resources into the shared pool (payout distributed by workflow)
  - DEFECT: keep your resources
  - Either way: pay 5 resources daily maintenance cost
"""
...
Your current resources: {resource_pool:.1f} (you die if resources fall below 20.0)
...
- COOPERATE: Contribute 15 resources. The total contributions are multiplied by 2.5 and split equally among all cooperators tomorrow.
- DEFECT: Contribute nothing. You keep your resources but receive no payout from the shared fund.
- Either way, you lose 5 resources per day for basic survival costs.
    ...
    async def forward_daily(self):
        ...
        try:
            ...
            if decision == "cooperate":
                resource_pool = max(0.0, resource_pool - 15.0)
            ...
            resource_pool = max(0.0, resource_pool - 5.0)
            ...
            # Fallback: apply daily cost only
            resource_pool = await self.memory.status.get("resource_pool", 100.0)
            resource_pool = max(0.0, resource_pool - 5.0)
            await self.memory.status.update("resource_pool", resource_pool)
            await self.memory.status.update("cooperated_this_round", False)
